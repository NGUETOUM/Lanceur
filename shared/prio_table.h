...
class PrioTable {
 public:
  ...
  int Attach(pid_t remote);
  int Attach(pid_t remote, int fd);
  void MarkUpdatedIndex(int idx, int num_retries);
  //int Attach(pid_t remote, int fd, uint32_t sched_items, uint32_t work_classes, uint32_t stream_capacity);
