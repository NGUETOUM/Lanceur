...
PrioTable::PrioTable(uint32_t num_items, uint32_t num_classes,
                     StreamCapacity stream_capacity, int fd) {
  uint32_t st_cap =
      static_cast<std::underlying_type<StreamCapacity>::type>(stream_capacity);
  size_t size = shmem_size(num_items, num_classes, st_cap);
  shmem_ = std::make_unique<GhostShmem>(kPrioTableVersion, kPrioTableShmemName,
...
/*PrioTable::PrioTable(uint32_t num_items, uint32_t num_classes,
                     StreamCapacity stream_capacity, pid_t remote) {
  uint32_t st_cap =
      static_cast<std::underlying_type<StreamCapacity>::type>(stream_capacity);
  size_t size = shmem_size(num_items, num_classes, st_cap);
  shmem_ = std::make_unique<GhostShmem>(kPrioTableVersion, kPrioTableShmemName,
...
int PrioTable::Attach(pid_t remote) {
...
int PrioTable::Attach(pid_t remote, int fd) {
  shmem_ = std::make_unique<GhostShmem>();
  int ret =
...
/*int PrioTable::Attach(pid_t remote, int fd, uint32_t sched_items, uint32_t work_classes, uint32_t stream_capacity) {
  shmem_ = std::make_unique<GhostShmem>();
  /*const bool ret =
      shmem_->Attach(kPrioTableVersion, kPrioTableShmemName, remote, fd);*/
 /*int ret = shmem_->Attach(kPrioTableVersion, kPrioTableShmemName, remote, fd, sched_items, work_classes, stream_capacity);
  if (ret < 0) {
    return ret;
  }
  hdr_ = reinterpret_cast<struct legorch_shmem_hdr*>(shmem_->bytes());
